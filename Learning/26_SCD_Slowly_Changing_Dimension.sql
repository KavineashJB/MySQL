...
-- Type 1   => overwrite on previous record
...
SET
    product_title = 'Amazon Echo Dot (3rd Gen)'
WHERE
    product_id = 101;

-- literally we don't need the wrong company name we need to override it.
select * from dim_product_type1;

-- Type 2   => Maintaining both previous and current record
...
-- If seller want to change their location.
-- First we need to update the current seller record which has is_current=1 to is_current=0 and end_date=NULL to end_date=current_timestamp
...
-- insert new record with new seller_key but same seller_id
INSERT INTO
    dim_seller_type2 (
        seller_key,
        seller_id,
...
-- Type 3   => Maintaining 2 cols to track prev and current value
