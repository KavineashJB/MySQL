-- Normalization in SQL is a systematic process of organizing the columns (attributes) and tables (relations) of a relational database to minimize data redundancy and improve data integrity. In simpler terms, normalization helps ensure that each piece of data is stored in the best possible place,
...
INSERT INTO
    StudentPhones (student_id, phone)
VALUES (1, '123-4567'),
    (1, '987-6543'),
    (2, '555-1212');
...
-- For  Example, If there are 2 primary keys in a 5 column then all other 2 column should depends on both primary key not a single(partial).
...
-- shouldn't change the order becaz we can't remove course table first becaz course_id reference is present in the enrollment table.
...
-- here the office depends on the officers not a course_id, even the instructor changes to another course the office of the respective instructor won't change
...
drop table if exists courses, Instructors;
